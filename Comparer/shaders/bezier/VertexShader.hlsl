...
bool clip(inout float4 B, inout float3 color_B, float2 distance_B, inout float3 position_B,
    ...
    if (any(min(sB, sC) < 0.0) || any(min(nB, nC) < 0.0)) {
        ...
        float2 d_B0 = distance_B, d_C0 = distance_C;
        distance_B = lerp(d_B0, d_C0, t0);
        distance_C = lerp(d_B0, d_C0, t1);

        float3 p_B0 = position_B, p_C0 = position_C;
        position_B = lerp(p_B0, p_C0, t0);
        position_C = lerp(p_B0, p_C0, t1);
...
CurveVSOutput main(uint index : SV_VertexID, uint i : SV_InstanceID) {
    ...
    float2 distance_B = Distances[i];
    float2 distance_C = Distances[i+1];
    ...
    if (!clip(B4, color_B, distance_B, position_B,
              C4, color_C, distance_C, position_C,
              A4, D4)) {
        o.Position = 0.0/0.0;
        return o;
    }
