...
{
  class AXODOX_COMMON_API Texture2D : public GraphicsResource
  {
  ...
  public:
    ...
    Texture2D(const GraphicsDevice& device, const winrt::com_ptr<ID3D11Texture2D>& texture, DXGI_FORMAT format = DXGI_FORMAT_UNKNOWN);
    ...
    void Clear(UINT*, GraphicsDeviceContext* context = nullptr);
    void Clear(FLOAT*, GraphicsDeviceContext* context = nullptr);
