...
namespace Axodox::Graphics
{
  ...
  void Texture2D::Clear(UINT* value, GraphicsDeviceContext* context)
  {
    if (!context) context = _device.ImmediateContext();

    Unbind(context);
    
    auto uav = GetUnorderedView({});
    if (uav)
    {
      (*context)->ClearUnorderedAccessViewUint(uav, value);
    }
  }

  void Texture2D::Clear(FLOAT* value, GraphicsDeviceContext* context)
  {
    if (!context) context = _device.ImmediateContext();
    ...
    if (uav)
    {
      (*context)->ClearUnorderedAccessViewFloat(uav, value);
    }
  }
