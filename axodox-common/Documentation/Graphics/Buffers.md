...
| Type | Purpose |
| --- | --- |
| `GraphicsBuffer` | Common base. Holds an `ID3D11Buffer` plus capacity / size, exposes the typed `Upload` template, and provides SRV/UAV binding for structured buffers. |
...
| `StructuredRW` | Read-write structured buffer with both `ID3D11ShaderResourceView` and `ID3D11UnorderedAccessView` for compute shaders and shader writes. |
| `BufferType` | Enum used by `GraphicsBuffer` (`Index`, `Vertex`, `Constant`, `Structured`, `StructuredRW`). |
...
classDiagram
  ...
  class GraphicsBuffer {
    ...
    -com_ptr~ID3D11UnorderedAccessView~ _unorderedAccessView
    +Upload(span~const uint8_t~, context)
    +Upload~T~(span~const T~, context)
    +BindShaderResourceView(stage, slot, context)
    +BindUnorderedAccessView(slot, context)
    +Unbind(context)
  }
...
For compute shaders that need to read and write data, use `BufferType::StructuredRW`, which provides both a shader resource view (SRV) and an unordered access view (UAV):
...
// Create a read-write buffer with capacity for 2048 elements
std::vector<OutputData> initialData(2048);
GraphicsBuffer rwBuffer{ device, TypedCapacityOrImmutableData<OutputData>{ initialData }, BufferType::StructuredRW, sizeof(OutputData) };

// Bind for compute shader write access
rwBuffer.BindUnorderedAccessView(/*slot*/ 0);         // bound as u0 in HLSL

// Dispatch compute shader that reads from input and writes to this buffer
computeShader.Run({ 256, 1, 1 });

// Clean up — this buffer can be bound elsewhere later
rwBuffer.UnbindUnorderedAccessView();
```

The `StructuredRW` buffer creates both the SRV and UAV internally. You can use `BindShaderResourceView` to read from it and `BindUnorderedAccessView` to write to it, but not simultaneously in the same shader stage. If you need both read and write access in a single shader, bind it as a UAV — the shader can still read through it.

### Binding structured buffers

For read-only structured buffers, bind the SRV to a shader stage:

```cpp
StructuredBuffer<Particle> input{ device, particles, sizeof(Particle) };
input.BindShaderResourceView(ShaderStage::Compute, /*slot*/ 0);
```

For read-write structured buffers, bind the UAV instead:

```cpp
GraphicsBuffer rwBuffer{ device, capacity, BufferType::StructuredRW, sizeof(OutputData) };
rwBuffer.BindUnorderedAccessView(/*slot*/ 0);
```

An SRV and UAV cannot be bound simultaneously on the same buffer. When you switch from SRV to UAV binding (or vice versa), the previous view is automatically unbound.
...
| File | Contents |
| --- | --- |
| [Graphics/Buffers/GraphicsBuffer.h](../../Axodox.Common.Shared/Graphics/Buffers/GraphicsBuffer.h) / [.cpp](../../Axodox.Common.Shared/Graphics/Buffers/GraphicsBuffer.cpp) | `GraphicsBuffer` base class, the `BufferType` enum (including `StructuredRW`), the `CapacityOrImmutableData` variant and its typed `TypedCapacityOrImmutableData<T>` wrapper, the templated `Upload<T>(span<const T>)` overload, and SRV/UAV binding methods for structured buffers. |
